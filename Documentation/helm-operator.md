...
| `csi.cephNFSFSGroupPolicy`           | Policy for modifying a volume's ownership or permissions when the CephNFS PVC is being mounted                               | ReadWriteOnceWithFSType                                   |
...
| `csi.cephNFSPluginTolerations`       | Array of tolerations in YAML format which will be added to CephCSI CephNFS plugin DaemonSet                                  | <none>                                                    |
| `csi.cephNFSPluginNodeAffinity`      | The node labels for affinity of the CephCSI CephNFS plugin DaemonSet (***)                                                   | <none>                                                    |
...
| `csi.csiCephNFSPluginResource`       | CEPH CSI CephNFS plugin resource requirement list.                                                                           | <none>                                                    |
...
| `csi.cephNFSPluginUpdateStrategy`    | CSI CephNFS plugin daemonset update strategy, supported values are OnDelete and RollingUpdate.                               | `RollingUpdate`                                                |
...
| `csi.nfs.image`       | NFS nodeplugin image. image.                                                                                        | `mcr.microsoft.com/k8s/csi/nfs-csi:v3.1.0`     |
