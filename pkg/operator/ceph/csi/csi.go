...
func (r *ReconcileCSI) setParams() error {
	...
	if EnableCephNFS, err = strconv.ParseBool(k8sutil.GetValue(r.opConfig.Parameters, "ROOK_CSI_ENABLE_CEPHNFS", "false")); err != nil {
		return errors.Wrap(err, "unable to parse value for 'ROOK_CSI_ENABLE_CEPHNFS'")
	}
	...
	csiCephNFSPodLabels := k8sutil.GetValue(r.opConfig.Parameters, "ROOK_CSI_CEPHNFS_POD_LABELS", "")
	CSIParam.CSICephNFSPodLabels = k8sutil.ParseStringToLabels(csiCephNFSPodLabels)
